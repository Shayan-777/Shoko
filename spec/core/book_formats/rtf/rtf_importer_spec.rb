  ...
  describe '#import' do
    context 'with Pride and Prejudice RTF', :requires_book_fixtures do
      ...
      it 'content can be parsed into blocks' do
        ...
        parser = Shoko::Core::BookFormats::Rtf::RtfContentParser.new(chapter.raw_content)
        blocks = parser.parse
    ...
    context 'FormatRegistry integration' do
      it 'is registered for .rtf' do
        cls = Shoko::Core::BookFormats::FormatRegistry.importer_for('test.rtf')
        expect(cls).to eq(described_class)
      end

      it 'reports .rtf as supported' do
        expect(Shoko::Core::BookFormats::FormatRegistry.supported_extension?('book.rtf')).to be true
      end
    end
  end
end
