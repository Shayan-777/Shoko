...
RSpec.describe 'Zero fallback outbound port contracts' do
  ...
  it 'defines DocumentLoader contract method' do
    implementation = build_implementation(Shoko::Core::Ports::Outbound::DocumentLoader)

    expect do
      implementation.load(path: '/tmp/book.epub', progress_reporter: nil, background_worker: nil)
    end.to raise_error(NotImplementedError)
  end

  it 'defines BackgroundWorkerBuilder contract method' do
    implementation = build_implementation(Shoko::Core::Ports::Outbound::BackgroundWorkerBuilder)
  ...
  it 'defines ProgressRepository contract methods' do
    implementation = build_implementation(Shoko::Core::Ports::Outbound::ProgressRepository)
