...
RSpec.describe Shoko::Adapters::BookSources::Kindle::MetadataParser do
  ...
  describe 'delegation' do
    it 'is used by KindleMetadataExtractor and preserves author_str output' do
      ...
      allow(Shoko::Adapters::BookSources::Kindle::PdbHeaderParser).to receive(:new).and_return(pdb)
      allow(Shoko::Adapters::BookSources::Kindle::MobiHeaderParser).to receive(:new).with('record0').and_return(mobi)
      allow(Shoko::Adapters::BookSources::Kindle::ExthParser).to receive(:new).and_return(exth)
      ...
      metadata = Shoko::Adapters::BookSources::Kindle::KindleMetadataExtractor.from_file(
        '/tmp/book_name.mobi',
