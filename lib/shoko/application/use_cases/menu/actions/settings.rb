      ...
      module Menu
        module Actions
          class Settings
            ...
            def initialize(menu_state_reader:, menu_session_mutator:, settings_service:, catalog:, navigation_actions:,
                           dictionary_actions:)
            ...
            def shift_settings_selection(delta)
              ...
              @menu_session_mutator.update_menu(settings_selected: (current + delta).clamp(0, max_index))
              :handled
            end
          end
        end
