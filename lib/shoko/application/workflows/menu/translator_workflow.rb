      ...
      module Menu
        ...
        class TranslatorWorkflow
          ...
          def fetch_languages(force: false)
            ...
            @async_relay.submit { perform_language_fetch }
            cached_languages
          end
          ...
          def translate_text(text:, source_lang:, target_lang:)
            ...
            @async_relay.submit { perform_translation(query, source_lang, target_lang) }
            nil
          end
          ...
          def perform_translation(query, source_lang, target_lang)
            result = @translation_service.translate(query, source_lang: source_lang, target_lang: target_lang)
            @async_relay.enqueue { update_translator_state(translation_payload(result)) }
          rescue Shoko::Error => e
            @async_relay.enqueue do
