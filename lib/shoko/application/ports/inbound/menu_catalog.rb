...
module Shoko
  module Application
    module Ports
      module Inbound
        # Canonical menu descriptors mapping cursor positions to inbound
        # intent symbols, alongside the presentation metadata (label/icon)
        # used by UI screens to render each entry.
        #
        # Conceptually an inbound contract: it tells the input adapter and
        # the screens which intents are available at each menu position.
        # Lives in `application/ports/inbound/` so both application use
        # cases (same layer) and UI screens (via the `application/ports/*`
        # carve-out) can reference it without violating layer boundaries.
        module MenuCatalog
          ...
          SETTINGS_ITEMS = [
            ...
            SettingsItem.new(action: :cycle_paragraph_style, icon_key: :line_spacing, label: 'Paragraph Style'),
            SettingsItem.new(action: :cycle_justify, icon_key: :line_spacing, label: 'Justify Text'),
            SettingsItem.new(action: :toggle_book_colors, icon_key: :theme, label: 'Book Colors'),
