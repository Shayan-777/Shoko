...
module Shoko
  module Shared
    module Terminal
      module TextMetrics
        # Measures strings in terminal cells: visible width, per-cluster cell
        # geometry, ANSI stripping, and tab expansion. Owns the text-lexing
        # constants shared by the truncator and wrapper.
        class Measurer
          ...
          def visible_length(text)
            source = text.to_s
            @cache.fetch(source) { measured_visible_length(source) }
          end
          ...
          def ascii_fast_path_enabled?
            @controls.ascii_fast_path_enabled?
          end

          # A string the byte-level ASCII truncation shortcut can handle:
          # pure ASCII with no escapes, tabs, or newlines.
          def fast_ascii_candidate?(str)
