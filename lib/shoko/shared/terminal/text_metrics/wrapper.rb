...
module Shoko
  module Shared
    module Terminal
      module TextMetrics
        # Wraps text to terminal cell widths: word-wrapping for plain text
        # and grapheme-cluster cell wrapping for oversized words and raw
        # streams.
        class Wrapper
          ...
          private_constant :PlainWrapState, :CellWrapState

          def initialize(measurer:, cache:)
            @measurer = measurer
            @cache = cache
          end
          ...
          def wrap_plain_text(line, width)
            source = line.to_s
            width_i = width.to_i
            @cache.lookup(source, width_i) { compute_wrapped_plain_text(source, width_i) }
          end
          ...
          def compute_wrapped_plain_text(source, width_i)
            normalized = @measurer.expand_tabs(source)
          ...
          def append_plain_wrap_word(state, word, width_i)
            ...
            word_width = @measurer.visible_length(word)
          ...
          def append_oversized_plain_word(state, word, width_i)
            ...
            if tail.empty?
              ...
              start_plain_wrap_word(state, tail, @measurer.visible_length(tail))
            end
          end
          ...
          def append_wrap_cell_cluster(state, cluster, max_width)
            cluster_width = @measurer.display_width_for(cluster)
